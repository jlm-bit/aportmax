...
# El expander se cierra (False) cuando ya hemos validado los datos
with st.expander("📝 TUS DATOS", expanded=not st.session_state.validado):
    ...
    st.write("---")
    if st.button(" Clica aqui para saber la aportacion adicional máxima posible que puedes aportar a tu PPE", use_container_width=True, type="primary"):
        if sb <= 0:
...
# --- 3. MURO DE SEGURIDAD ---
# Si el sueldo es 0 o no se ha validado, detenemos la ejecución aquí.
if sb <= 0 or not st.session_state.validado:
    st.info("👋 Introduce tus datos arriba para activar el cálculo.")
    st.stop()

# --- 4. LÓGICA DE CÁLCULO (Se ejecuta solo si pasamos el st.stop) ---
...
# --- 5. TABS Y RESULTADOS ---
tab1, tab2, tab3 = st.tabs(["Aportación Máxima", "Proyección", "Acerca de"])

with tab1:
    # Ahora esta variable YA EXISTE porque se calculó en el punto 4
    ...
    with st.expander("🔍 Ver Detalle "):
        st.markdown(f"""
            <div style="font-size: 0.9rem; color: #475569; line-height: 1.6;">
                El importe de <b>{aportacion_extraordinaria_neta:,.2f} €</b> representa la aportación adicional a tu PPE que aún puedes aprovechar este año. Se calcula restando tus aportaciones realizadas y planificadas a tu límite máximo permitido:
                <ul style="margin-top: 10px;">
                    <li><b>Aportación Total (incluyendo la aportacion del Promotor:</b> {total_inversion:,.2f} €</b>. Considerando las contribuciones estimadas de la empresa a tu PPE ({emp_t:,.2f} €).</li>
                    <li><b>Límite máximo personal:</b> {max_p:,.2f} €</b>. 
                    <li><b>Aportaciones ya realizadas:</b>  {ya_aportado:,.2f} € ya aportados hasta ahora.</li>
                    <li><b>Aportaciones planificación futuras:</b>  {total_mensual_previsto:,.2f} € que tienes previsto aportar mediante deducción en nómina o aportaciones periódicas hasta diciembre.</li>
...
with st.expander("ℹ️ Tu Plan de Acción", expanded=False):
    ...
    with col1:
        st.markdown(f"""
            <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; padding: 20px; border-radius: 12px; height: 200px;">
